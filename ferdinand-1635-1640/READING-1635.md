...
| 13 | P2.L03 | XZ: `i u 9 p q 32 z` → `r⟦u⟧berna`. Draft *hiberna*. **Resolved on the image, 18 Sept 2026:** the first sign carries the top-left flag of this hand's digit 1 and has no i-dot (compare `1` in L07 and the dotted `i` in L12); the u-shape is two flagged strokes joined at the foot, the digit 11 written without lifting the pen (compare the separate `11` in P2.L02). Working transcription corrected to `1 11 9 p q 32 z`; XZ's reading is kept in the file header. | *hiberna*, now literal output under the unchanged key (`1`=h, `11`=i). Grades for the two units move from M to C. |
