...
|---|---|---|---|
| 1 | P1.L11 | `x 12 y 15 37 7 31` → `ciuitcs`; arc below final signs also noted by XZ. Draft main column *ciuitas*. | `[civitas]`; the 7-like sign is not changed to an a-symbol in the data. Could be correction mark/glyph ambiguity or an inherited enciphering mistake. |
| 2 | P1.L12, L19, L23; P2.L02 | The sign transcribed `6` yields *e* in training *Caesareo/Trevirensi*, but *m* would be required in *iam, praesidium, tum, militi*. | Mark restorations. A b/6 graphic distinction is a specific hypothesis, not a demonstrated context-sensitive substitution. Main decoder retains the frozen e-value. |
| 3 | P1.L15–16 | XZ `20 0 0 m` was visually re-read `20 c o m`; final sequence → `praesidio`. Draft margin also has `20 c o m`. | Documented transcription correction, not a cipher emendation. The new mapping `o=a` is post-validation F04. |
...
| 7 | P1.L20 | `locisqce`, with 7-like sign producing *c*. Draft *locisq*. | `[locisque]`; y/7 confusion is possible but not established. |
...
| 11 | P1.L24 | `cuihctenus`, lacks a letter for *a*. Draft has inserted *hactenus*. | `[hactenus]`, supplied *a*. No invisible cipher symbol added. |
...
| 13 | P2.L03 | `i u 9 p q 32 z` → `r⟦u⟧berna`; first two glyphs could instead be h/i-encoding variants (1/11-like). Draft explicitly *hiberna*. | `[hiberna]` entirely flagged. No new `u=i` mapping selected just to produce the word. |
...
Later comparative evidence: the 1640 reading supports distinct labels `b=m` and `6=e` in that witness. This helps frame note 2 but does not retrospectively settle the corresponding ambiguous 1635 glyphs. The original frozen key and first-pass holdout remain unchanged.
