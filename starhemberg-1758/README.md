...
**Grades** ([CONVENTIONS.md](../CONVENTIONS.md)): of 423 parsed units, 417 H (378 ordinary units found in the 1752 Prima/Secunda tables, plus 39 read from the rest of the surviving key material: 33 nulls, which the Nota of 3 September 1752 defines, and 4 punctuation marks and 2 table switches listed in the numeric tables' 800 column; sources table below) and 6 M (absent from the working key or malformed); every digit repair is I and stays in [repair-cases.json](repair-cases.json), never in the baseline. [two-table-parsing.json](two-table-parsing.json) carries the grade per unit.
...
| Null rule | [R1696](https://de-crypt.org/decrypt-web/RecordsView/1696) image I7480 P1, the Nota of 3 September 1752, items 2 and 4: in both tables the 8 is the *errans* and the 3 announces four following digits; the 8 goes only among the two-digit numbers, never inside a four-digit code. Both numeric tables ([R1698](https://de-crypt.org/decrypt-web/RecordsView/1698) I7484 P1 and I7486 P3) repeat it in their heading, *8 errans cum duobus numeris tantum*. Read on the images 19 September 2026. |
| Marked codes | The 800 column of each numeric table, [R1698](https://de-crypt.org/decrypt-web/RecordsView/1698): Clavis Prima, I7484 P1, has 854 *coma* and 876 *indic. Clav. Secundam*; Clavis Secunda, I7486 P3, has 841 *Coma*, 870 *Coma*, 897 *punctum* and 899 *indic. clav. Primam*. Nota item 6 (I7480 P1) puts the mark over the middle digit. Read on the images 19 September 2026; [two-table-parsing.json](two-table-parsing.json) names the source per unit in `basis`. |
